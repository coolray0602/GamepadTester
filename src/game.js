		...
		if(gamepads[focusGamepad]!==null && gamepads[focusGamepad].connected){		
			...
			if(!this.roundnessTest){
				...
				for(var j=0;j<=31;j++){
					...
					ctx.beginPath();
					if(this.roundArray[j]<1){
						ctx.fillStyle = "rgba("+600*(1-this.roundArray[j])+", "+5*this.roundArray[j]+", "+10*(this.roundArray[j]-1)+", 0.5)";
					}else if(this.roundArray[j]>outDeadzone){	//outer round limitation
						ctx.fillStyle = "rgba("+10*(1-this.roundArray[j])+", "+5*this.roundArray[j]+", "+200*(this.roundArray[j])+", 0.5)";
					}else{
						ctx.fillStyle = "rgba("+600*(1-this.roundArray[j])+", "+200*this.roundArray[j]+", "+1000*(this.roundArray[j]-1)+", 0.5)";
					}
				...
				for(var j=0;j<=31;j++){
					...
					if(this.roundArray[j]<1 || this.roundArray[j]>outDeadzone){
						ctx.fillStyle="red";
						roundPass=false;
					}else {
						ctx.fillStyle="green";
