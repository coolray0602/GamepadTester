...
var linearityRange = parseFloat(document.getElementById("testRange").value)/100.0; //測試線性的百分比，0.6表示只計算60%以內的輸出
...
linearityTestBtn.onclick = function(){
	if(checkGamepadsConnected()){
		if(linearityTesting==false){
			linearityRange=document.getElementById("testRange").value/100.0; //重新讀取測試範圍
...
function drawLinearityLine(adding,linearityArray,ctx){
	//setTimeout(returnAxis,1000);
	//畫去向
	//console.log("開始畫去向");
	var maxErr=0;
	var outputErr=0;
	var end;
	if(adding){
		end=linearityArray.findIndex(element => element > linearityRange);//.indexOf(Math.max.apply(Math, linearityArray))-1;
		console.log("有效樣本數："+end);
	}else{
		end=linearityArray.findIndex(element => element < -linearityRange);//.indexOf(Math.min.apply(Math, linearityArray))-1;
		console.log("有效樣本數："+end);
	}
	
	var shift=linearityArray[0];
	var tilt=1.0/(linearityArray[end]-shift);
	var gap=(linearityCanvasSize/2)/end;

	ctx.beginPath();
	ctx.moveTo(linearityCanvasSize/2,linearityCanvasSize/2);

	for (i=0;i<end;i++){
		ctx.beginPath();

		if(adding){
			ctx.moveTo(linearityCanvasSize/2+i*gap,linearityCanvasSize/2-((linearityArray[i]-shift)*tilt*linearityCanvasSize/2));
		}else{
			ctx.moveTo(linearityCanvasSize/2-i*gap,linearityCanvasSize/2+((linearityArray[i]-shift)*tilt*linearityCanvasSize/2));
		}
		outputErr=Math.abs((linearityArray[i]-shift)*tilt*linearityCanvasSize/2-i*gap)/linearityCanvasSize*100;
		//console.log("outputErr="+outputErr);
		if(Math.abs(outputErr)> maxErr) maxErr=Math.abs(outputErr);

		if(outputErr>linearityStd){
			ctx.strokeStyle="red";				
		}else {
			ctx.strokeStyle="green";
		}
		if(linearityArray[i+1]<=linearityArray[i]){
			linearityArray[i+1]=(linearityArray[i]+linearityArray[i+2])/2;
		}
		if(adding){
			ctx.lineTo(linearityCanvasSize/2+(i+1)*gap,linearityCanvasSize/2-((linearityArray[i+1]-shift)*tilt*linearityCanvasSize/2));
		}else{
			ctx.lineTo(linearityCanvasSize/2-(i+1)*gap,linearityCanvasSize/2+((linearityArray[i+1]-shift)*tilt*linearityCanvasSize/2));
...
function drawLinearity(){
	...
	if(xRight==Math.max(xRight,xLeft,yUp,yDown)){
		setTimeout("xReturn1=gamepads[focusGamepad].axes[2]",1000);
		maxErr=drawLinearityLine(true,xLinearityArray,x1linearityCtx);
		if(maxErr>maxErrX1) maxErrX1=maxErr;
		document.getElementById("resultX1").innerHTML="線性誤差："+maxErrX1.toFixed(2)+"%";
		xLinearityArray.reverse();
		maxErr=drawLinearityLine(true,xLinearityArray,x2linearityCtx);
		if(maxErr>maxErrX2) maxErrX2=maxErr
		document.getElementById("resultX2").innerHTML="線性誤差："+maxErrX2.toFixed(2)+"%";
		phaseDone++;
		console.log("完成x向右線性測試");
	}else if(xLeft==Math.max(xRight,xLeft,yUp,yDown)){
		setTimeout("xReturn2=gamepads[focusGamepad].axes[2]",1000);
		maxErr=drawLinearityLine(false,xLinearityArray,x1linearityCtx);
		if(maxErr>maxErrX1) maxErrX1=maxErr;
		document.getElementById("resultX1").innerHTML="線性誤差："+maxErrX1.toFixed(2)+"%";
		xLinearityArray.reverse();
		maxErr=drawLinearityLine(false,xLinearityArray,x2linearityCtx);
		if(maxErr>maxErrX2) maxErrX2=maxErr
		document.getElementById("resultX2").innerHTML="線性誤差："+maxErrX2.toFixed(2)+"%";
		phaseDone++;
		console.log("完成x向左線性測試");
	}else if(yUp==Math.max(xRight,xLeft,yUp,yDown)){
		setTimeout("yReturn1=gamepads[focusGamepad].axes[3]",1000);
		maxErr=drawLinearityLine(false,yLinearityArray,y1linearityCtx);
		if(maxErr>maxErrY1) maxErrY1=maxErr;
		document.getElementById("resultY1").innerHTML="線性誤差："+maxErrY1.toFixed(2)+"%";
		yLinearityArray.reverse();
		maxErr=drawLinearityLine(false,yLinearityArray,y2linearityCtx);
		if(maxErr>maxErrY2) maxErrY2=maxErr
		document.getElementById("resultY2").innerHTML="線性誤差："+maxErrY2.toFixed(2)+"%";
		phaseDone++;
		console.log("完成y向上線性測試");
	}else if(yDown==Math.max(xRight,xLeft,yUp,yDown)){
		setTimeout("yReturn2=gamepads[focusGamepad].axes[3]",1000);
		maxErr=drawLinearityLine(true,yLinearityArray,y1linearityCtx);
		if(maxErr>maxErrY1) maxErrY1=maxErr;
		document.getElementById("resultY1").innerHTML="線性誤差："+maxErrY1.toFixed(2)+"%";
		yLinearityArray.reverse();
		maxErr=drawLinearityLine(true,yLinearityArray,y2linearityCtx);
		if(maxErr>maxErrY2) maxErrY2=maxErr
		document.getElementById("resultY2").innerHTML="線性誤差："+maxErrY2.toFixed(2)+"%";
		phaseDone++;
	...
	if(phaseDone>=4){
		...
		if(maxErrX1>linearityStd)  document.getElementById("xGo").style.backgroundColor="red";			
		if(maxErrX2>linearityStd) document.getElementById("xBack").style.backgroundColor="red";
		if(maxErrY1>linearityStd) document.getElementById("yGo").style.backgroundColor="red";
		if(maxErrY2>linearityStd) document.getElementById("yBack").style.backgroundColor="red";
		maxErrX1=0;
		maxErrX2=0;
		maxErrY1=0;
		maxErrY2=0;
